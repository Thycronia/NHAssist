/*namespace NHAssist.UIs.UIBases
{
	public class UIScrollbar : UIElement
	{
		...
		public UIScrollbar(float width, Asset<Texture2D> texture = null, Asset<Texture2D> innerTexture = null)
        {
			Width.Set(width, 0f);
			MaxWidth.Set(width, 0f);
			_texture = texture ?? Main.Assets.Request<Texture2D>("Images/UI/Scrollbar");
			_innerTexture = innerTexture ?? Main.Assets.Request<Texture2D>("Images/UI/ScrollbarInner");
			PaddingTop = 5f;
			PaddingBottom = 5f;
		}
		...
		private Rectangle GetHandleRectangle()
		{
			...
			return new Rectangle((int)innerDimensions.X, (int)(innerDimensions.Y + innerDimensions.Height * (_viewPosition / _maxViewSize)) - 3, (int)Width.Pixels, (int)(innerDimensions.Height * (_viewSize / _maxViewSize)) + 7);
		}
